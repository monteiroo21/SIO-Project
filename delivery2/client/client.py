...
def parse_env(state):
    ...
    if 'REP_PUB_KEY' in os.environ:
        ...
        logger.debug("REP_PUB_KEY not found in environment, loading default key")
...
def parse_args(state):
    ...
    parser.add_argument("-d", "--date", nargs=2, help="Date")
    ...
    if args.key:
        ...
        with open(key_file, 'r') as f:
            state['REP_PUB_KEY'] = f.read()
            logger.info(f'Repository public key loaded from: {key_file}')
    elif 'REP_PUB_KEY' in os.environ:
        key_file = os.getenv('REP_PUB_KEY')
        if not os.path.exists(key_file) or not os.path.isfile(key_file):
            logger.error(f'Environment variable REP_PUB_KEY points to an invalid file: {key_file}')
            sys.exit(-1)
        with open(key_file, 'r') as f:
            state['REP_PUB_KEY'] = f.read()
            logger.info(f'Repository public key loaded from environment variable: {key_file}')

    if args.repo:
        repo_address = args.repo[0]
        state['REP_ADDRESS'] = repo_address
        logger.info(f'Repository address set to: {repo_address}')
    elif 'REP_ADDRESS' in os.environ:
        state['REP_ADDRESS'] = os.getenv('REP_ADDRESS')
        logger.info(f'Repository address loaded from environment variable: {state["REP_ADDRESS"]}')
    
    if args.command:
        logger.info("Command: " + args.command)
       
    return state, {'command': args.command, 'arg0': args.arg0, 'arg1': args.arg1, 'arg2': args.arg2, 'arg3': args.arg3, 'arg4': args.arg4, 'arg5': args.arg5, 'user':args.username, 'date':args.date}
...
def rep_subject_credentials(arg0, arg1):
    if arg1 is  None:
        logger.error("rep_subject credentials: Missing argument")
        return 1
    logger.debug(f"Executing rep_subject_credentials with arg0={arg0} and arg1={arg1}")

    password = arg0.encode()
    credentials_file = arg1

    key_gen(password,credentials_file)
    logger.debug("Credentials generated successfully") 
    return 0
...
def rep_drop_role(session_file, role):
    logger.debug("rep_drop_role: Dropping role")
    ...
    try:
        ...
        request_data = encryptMessageWithSession(request_data, session)
        logger.debug("Sending drop role request to repository")
        response = requests.post(f"http://{repo_address}/role/drop", json=request_data)

        if response.status_code == 200:
            logger.info("Role dropped successfully")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def rep_list_roles(session_file):
    logger.debug("rep_list_roles: Listing roles")
    ...
    try:
        ...
        request_data = encryptMessageWithSession(request_data, session)
        logger.debug("Sending list roles request to repository")
        response = requests.get(f"http://{repo_address}/role/list", json=request_data)
...
def rep_assume_role(session_file, role):
    logger.debug("rep_assume_role: Assuming role")
    try:
        logger.debug(f"Opening session file '{session_file}'")
        with open(session_file, "r") as file:
            session = json.loads(file.read())
    ...
    try:
        ...
        request_data = encryptMessageWithSession(request_data, session)
        logger.debug("Sending assume role request to repository")
        response = requests.post(f"http://{repo_address}/role/assume", json=request_data)
...
def rep_list_role_subjects(session_file, role):
    logger.debug("rep_list_role_subjects: Listing role subjects")
    ...
    try:
        ...
        request_data = encryptMessageWithSession(request_data, session)
        logger.debug("Sending list role subjects request to repository")
        response = requests.get(f"http://{repo_address}/role/list_subjects", json=request_data)

        if response.status_code == 200:
            logger.info("Role subjects listed successfully")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
            ...
            logger.error(json.dumps({
                "error": "Failed to list role subjects",
                "status_code": response.status_code,
                "details": response.text
            }, indent=4))
            return -1
...
def rep_list_subject_roles(session_file, username):
    logger.debug("rep_list_subject_roles: Listing subject roles")
    ...
    try:
        ...
        request_data = encryptMessageWithSession(request_data, session)
        logger.debug("Sending list subject roles request to repository")
        response = requests.get(f"http://{repo_address}/subject/list_roles", json=request_data)

        if response.status_code == 200:
            logger.info("Subject roles listed successfully")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
            ...
            logger.error(json.dumps({
                "error": "Failed to list subject roles",
                "status_code": response.status_code,
                "details": response.text
            }, indent=4))
            return -1
...
def rep_list_role_permissions(session_file, role):
    logger.debug("rep_list_role_permissions: Listing role permissions")
    ...
    try:
        ...
        request_data = encryptMessageWithSession(request_data, session)
        logger.debug("Sending list role permissions request to repository")
        response = requests.get(f"http://{repo_address}/role/list_permissions", json=request_data)

        if response.status_code == 200:
            logger.info("Role permissions listed successfully")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def rep_list_permission_roles(session_file, permission):
    logger.debug("rep_list_permission_roles: Listing permission roles")
    ...
    try:
        ...
        request_data = encryptMessageWithSession(request_data, session)
        logger.debug("Sending list permission roles request to repository")
        response = requests.get(f"http://{repo_address}/permission/list_roles", json=request_data)

        if response.status_code == 200:
            logger.info("Permission roles listed successfully")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
            ...
            logger.error(json.dumps({
                "error": "Failed to list permission roles",
                "status_code": response.status_code,
                "details": response.text
            }, indent=4))
            return -1
...
def rep_add_role(session_file, role):
    ...
    try:
        ...
        logger.debug("Sending add role request to repository")
        response = requests.post(f"http://{repo_address}/role/add", json=request_data)

        if response.status_code == 200:
            logger.info(f"Role '{role}' added successfully")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def rep_suspend_role(session_file, role):
    logger.debug("rep_suspend_role: Suspending role")
    try:
        logger.debug(f"Opening session file '{session_file}'")
        with open(session_file, "r") as file:
            session = json.loads(file.read())
        ...
        request_data = encryptMessageWithSession(data, session)
        logger.debug("Sending suspend role request to repository")
        response = requests.post(f"http://{repo_address}/role/suspend", json=request_data)

        if response.status_code == 200:
            logger.info(f"Role '{role}' suspended successfully.")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def rep_reactivate_role(session_file, role):
    logger.debug("rep_reactivate_role: Reactivating role")
    try:
        logger.debug(f"Opening session file '{session_file}'")
        with open(session_file, "r") as file:
            session = json.loads(file.read())
        ...
        request_data = encryptMessageWithSession(data, session)
        logger.debug("Sending reactivate role request to repository")
        response = requests.post(f"http://{repo_address}/role/reactivate", json=request_data)

        if response.status_code == 200:
            logger.info(f"Role '{role}' reactivated successfully.")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def rep_add_permission(session_file, role, arg03):
    logger.debug("rep_add_permission: Adding permission")
    try:
        logger.debug(f"Opening session file '{session_file}'")
        with open(session_file, "r") as file:
            session = json.loads(file.read())
        ...
        request_data = encryptMessageWithSession(data, session)
        logger.debug("Sending add permission request to repository")
        response = requests.post(f"http://{repo_address}/permission/add", json=request_data)

        if response.status_code == 200:
            logger.info(f"Permission '{arg03}' added successfully.")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def rep_remove_permission(session_file, role, arg03):
    logger.debug("rep_remove_permission: Removing permission")
    try:
        logger.debug(f"Opening session file '{session_file}'")
        with open(session_file, "r") as file:
            session = json.loads(file.read())
        ...
        request_data = encryptMessageWithSession(data, session)
        logger.debug("Sending remove permission request to repository")
        response = requests.post(f"http://{repo_address}/permission/remove", json=request_data)

        if response.status_code == 200:
            logger.info(f"Permission '{arg03}' removed successfully.")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def rep_acl_doc(session_file, doc_name, signal, role, permission):
    logger.debug("rep_acl_doc: ACL document")
    state = load_state()
    ...
    try:
        ...
        with open(session_file, "r") as file:
            ...
            session_id=session["session_id"]
        if signal not in ["+","-"]:
            logger.info("Invalid operation. Use '+' to add and '-' to remove.")
            return 1
        data = {
            "session": session_id,
            "doc_name": doc_name,
        ...
        request_data = encryptMessageWithSession(data, session)
        logger.debug("Sending acl documents request to repository")
        response = requests.post(f"http://{repo_address}/document/acl", json=request_data)

        if response.status_code == 200:
            logger.info(f"Signal {signal} on role {role} and permission {permission} for document {doc_name} successful.")
            data = response.json()
            data = decryptDataWithSession(data, session,session_file)
            logger.info(json.dumps(data, indent=4))
            return 0
...
def main():
    ...
    match args["command"]:
        ...
        case "rep_assume_role":
            exitCode = rep_assume_role(args["arg0"], args["arg1"])
        case "rep_drop_role":
            exitCode = rep_drop_role(args["arg0"], args["arg1"])
        case "rep_list_roles":
            exitCode = rep_list_roles(args["arg0"])
        case "rep_list_role_subjects":
            exitCode = rep_list_role_subjects(args["arg0"], args["arg1"])
        case "rep_list_subject_roles":
            exitCode = rep_list_subject_roles(args["arg0"], args["arg1"])
        case "rep_list_role_permissions":
            exitCode = rep_list_role_permissions(args["arg0"], args["arg1"])
        case "rep_list_permission_roles":
            exitCode = rep_list_permission_roles(args["arg0"], args["arg1"])
        case "rep_add_role":
            exitCode = rep_add_role(args["arg0"], args["arg1"])
        case "rep_suspend_role":
            exitCode = rep_suspend_role(args["arg0"], args["arg1"])
        case "rep_reactivate_role":
            exitCode = rep_reactivate_role(args["arg0"], args["arg1"])
        case "rep_add_permission":
            exitCode = rep_add_permission(args["arg0"], args["arg1"], args["arg2"])
        case "rep_remove_permission":
            exitCode = rep_remove_permission(args["arg0"], args["arg1"], args["arg2"])
        case "rep_acl_doc":
            exitCode = rep_acl_doc(args["arg0"], args["arg1"], args["arg2"], args["arg3"], args["arg4"])          
        case _:
