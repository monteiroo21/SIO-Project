...
def encrypt_string(string, credential_file, password):
    private_key, public_key = key_load(password, credential_file)
    
    ciphertext, nonce, tag, salt = encrypt_data(string.encode(), private_key, public_key)
    
    return ciphertext.hex(), nonce.hex(), tag.hex(), salt.hex()

def decrypt_string(ciphertext, nonce, tag, salt, credential_file, password):
    private_key = load_private_key(password, credential_file)
    public_key = private_key.public_key()
    ...
    decrypted_string = decrypt_data(ciphertext_bytes, nonce_bytes, tag_bytes, salt_bytes, private_key, public_key)
